...
class Support extends MY_Controller {
    ...
    public function add_ticket_note()
    {
        ...
        if ( $this->form_validation->run( 'support_note' ) )
        {
            ...
            if ( ! empty( $id ) )
            {
                ...
                $this->zwebhook->ticket_event('note_added', $ticket, [
                    'note_id' => $id,
                    'user_id' => $user_id
                ]);
        ...
        if ( $this->Support_model->assign_user( $data_id, $user_id, $type . 's' ) )
        {
            if ( $user_id !== null )
            {
                if ( $type === 'ticket' )
                {
                    ...
                    // Send webhook notification
                    $ticket = $this->Support_model->ticket( $data_id );
                    $this->zwebhook->ticket_event('assigned', $ticket, [
                        'assigned_to' => $user_id,
                        'assigned_by' => 'admin'
                    ]);
                }
    ...
    public function add_reply()
    {
        ...
        if ( post( 'solved' ) && ! post( 'reply' ) )
        {
            if ( $ticket->sub_status != 3 )
            {
                ...
                // Send webhook notification
                $ticket = $this->Support_model->ticket( $ticket_id );
                $this->zwebhook->ticket_event('solved', $ticket, [
                    'solved_by' => 'admin'
                ]);
        ...
        if ( $this->form_validation->run( 'add_reply' ) )
        {
            ...
            if ( ! empty( $id ) )
            {
                ...
                // Send webhook notification
                $this->zwebhook->ticket_event('reply_added', $ticket, [
                    'reply_id' => $id,
                    'replied_by' => 'admin'
                ]);
    ...
    public function create_ticket()
    {
        ...
        if ( $this->form_validation->run( $key ) )
        {
            ...
            if ( ! empty( $id ) )
            {
                ...
                // Send webhook notification (after custom fields are saved)
                $ticket = $this->Support_model->ticket( $id );
    ...
    public function delete_reply()
    {
        ...
        if ( $this->Support_model->delete_ticket_reply( $id ) )
        {
            ...
            // Send webhook notification
            $ticket = $this->Support_model->ticket( $reply->ticket_id );
            $this->zwebhook->ticket_event('reply_deleted', $ticket, [
                'reply_id' => $id
            ]);
